...
namespace Headquarters4DCS
{
    ...
    public struct MinMaxD
    {
        ...
        public MinMaxD(double min, double max) { Min = Math.Min(min, max); Max = Math.Max(min, max); }
        ...
        public MinMaxD(string minMaxString)
        {
            try
            {
                string[] minAndMaxString = minMaxString.Split(',');

                double val1 = HQTools.StringToDouble(minAndMaxString[0]);
                double val2 = HQTools.StringToDouble(minAndMaxString[1]);
        ...
        public double GetValue() { return HQTools.RandomDouble(Min, Max); }
        ...
        public bool Contains(double value) { return (value >= Min) && (value <= Max); }
    }
}
