...
namespace Headquarters4DCS.Generator
{
    public sealed class MissionGeneratorTemplateChecker : IDisposable
    {
        ...
        private void CheckCoalitions(MissionTemplate template)
        {
            ...
            coals[(int)Coalition.Blue] = Library.Instance.GetDefinition<DefinitionCoalition>(template.Settings.ContextCoalitionBlue);
            coals[(int)Coalition.Red] = Library.Instance.GetDefinition<DefinitionCoalition>(template.Settings.ContextCoalitionRed);

            for (int i = 0; i < 2; i++)
            {
                if ((coals[i].MinMaxTimePeriod[0] > template.Settings.ContextTimePeriod) || (coals[i].MinMaxTimePeriod[1] < template.Settings.ContextTimePeriod))
                    throw new HQ4DCSException($"Coalition {coals[i].DisplayName} is not available in the {template.Settings.ContextTimePeriod.ToString().Replace("Decade", "")}s.");
            }
        ...
        {
            if (!Library.Instance.DefinitionExists<DefinitionCoalition>(template.Settings.ContextCoalitionBlue))
                throw new HQ4DCSException($"Coalition \"{template.Settings.ContextCoalitionBlue}\" not found.");

            if (!Library.Instance.DefinitionExists<DefinitionCoalition>(template.Settings.ContextCoalitionRed))
                throw new HQ4DCSException($"Coalition \"{template.Settings.ContextCoalitionRed}\" not found.");

            if (!Library.Instance.DefinitionExists<DefinitionLanguage>(template.Settings.PreferencesLanguage))
                throw new HQ4DCSException($"Language \"{template.Settings.PreferencesLanguage}\" not found.");

            if (!Library.Instance.DefinitionExists<DefinitionTheater>(template.Theater))
                throw new HQ4DCSException($"Theater \"{template.Theater}\" not found.");
        }
    }
}
